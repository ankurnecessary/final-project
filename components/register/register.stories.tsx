...
export const Default: Story = {
  play: async ({ canvasElement }) => {
    ...
    await waitFor(() => expect(screen.queryByText('Name is required')).toBeInTheDocument());
    ...
    await waitFor(() => expect(screen.queryByText('Confirm Password is required')).toBeInTheDocument());
    ...
    await waitFor(() => expect(screen.queryByText('At least 8 characters')).toBeInTheDocument());
    ...
    // 👇 Assert Password validation message not there
    await waitFor(() => expect(screen.queryByText('Password must include at least 1 upper case letter.')).toBeInTheDocument());
    ...
    // 👇 Assert Password validation message not there
    await waitFor(() => expect(screen.queryByText('Password must include at least 1 number.')).toBeInTheDocument());
    ...
    // 👇 Assert Password validation message not there
    await waitFor(() => expect(screen.queryByText('Password must include at least 1 special character.')).not.toBeInTheDocument());
